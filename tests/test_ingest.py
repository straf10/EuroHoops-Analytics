...
from eurohoops.ingest.http import MAX_ATTEMPTS, MIN_INTERVAL_S, Fetcher
...
def fetcher_for(api: FakeApi) -> Fetcher:
    return Fetcher(httpx.Client(transport=httpx.MockTransport(api)))


@pytest.mark.usefixtures("no_sleep")
def test_second_ingest_of_completed_season_makes_zero_http_calls(tmp_path: Path) -> None:
    first = FakeApi()
    schedules = ingest_seasons(fetcher_for(first), tmp_path, [2024], details=True)
    assert len(schedules[2024]) == 3
    assert len(first.calls) == 1 + 3 * 3  # schedule + 3 endpoints x 3 played games

    second = FakeApi()
    again = ingest_seasons(fetcher_for(second), tmp_path, [2024], details=True)
    assert second.calls == []
    assert again == schedules


@pytest.mark.usefixtures("no_sleep")
def test_incomplete_season_schedule_is_refreshed_and_unplayed_games_get_no_details(
    tmp_path: Path,
) -> None:
    ingest_seasons(fetcher_for(FakeApi()), tmp_path, [2026], details=True)
    api = FakeApi()
    ingest_seasons(fetcher_for(api), tmp_path, [2026], details=True)
...
def test_raw_cache_layout_is_gzip_and_file_existence_based(tmp_path: Path) -> None:
    ingest_seasons(fetcher_for(FakeApi()), tmp_path, [2024], details=True)
...
def test_empty_detail_bodies_are_not_cached(tmp_path: Path) -> None:
    ingest_seasons(fetcher_for(FakeApi(empty_details=True)), tmp_path, [2024], details=True)
...
def test_schedules_only_without_details_flag(tmp_path: Path) -> None:
    api = FakeApi()
    ingest_seasons(fetcher_for(api), tmp_path, [2024], details=False)
    assert len(api.calls) == 1
...
def test_fetcher_gives_up_after_max_attempts(no_sleep: list[float]) -> None:
    ...
    with pytest.raises(httpx.ConnectError):
        Fetcher(httpx.Client(transport=httpx.MockTransport(handler))).get("https://x.test/")
