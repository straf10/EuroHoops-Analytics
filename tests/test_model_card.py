"""E8: every number in the M1 model card matches the committed report JSON."""
...
CARD = REPO / "docs" / "models" / "m1.md"
ROW = re.compile(
    r"^\| (?P<label>[^|]+) \| (?P<value>[^|]+) \| `(?P<file>[\w.]+):(?P<path>[\w.]+)` \|$"
)
DECIMAL = re.compile(r"-?\d+\.\d{3,}")
REPORTS = ("backtest_m1.json", "backtest_m1_gbl.json", "possessions.json", "stints_mart.json")
...
def test_every_decimal_in_the_prose_is_a_table_value() -> None:
    ...
    table_values = {r["value"].strip() for r in card_rows()}
    missing = sorted(
        {
            n
