...
Declared before any run (weeks 7–10b G5, `reports/week7-10b_progress.md`, LEVEL_DECLARATION)
to target the calibration failure: `lgbm_level` and `spline_level` add a season offset to the
base model's logit. For a shot in game g, the offset mixes the season's own level so far (the
maximum-likelihood shift over the season's shots in games that tipped off before g) with the
previous season's full-season shift, weight n / (n + k) for n shots so far; k is fitted by
empirical Bayes on development seasons (2312 shots for `lgbm_level`: after about 15 games the
season's own level gets half the weight). No offset uses a shot of its own game or a later one
(leakage tests). **Validation and test were seen before this variant existed, so every number
here is post-hoc, not a clean hold-out**; the clean test is the live 2026-27 season, scored
once after it ends by a pre-registered rule (`scripts/checks/m2_live_level.py`).

| | lgbm | lgbm_level | spline | spline_level |
|---|---|---|---|---|
| CV log loss | 0.629699 | 0.629697 | 0.631577 | 0.631519 |
| Validation log loss | 0.633267 | 0.633309 | 0.635097 | 0.635106 |
| Validation ECE | 0.0104 | 0.0102 | 0.0092 | 0.0097 |
| Test ECE | 0.0126 | 0.0095 | 0.0119 | 0.0120 |

**Result: `lgbm_level` does not meet F-f on validation** (ECE 0.0102, target 0.01) and does not
improve log loss (level − lgbm on validation +0.000042, 95% CI [−0.000049, 0.000139]). What it
does fix is the league level: calibration in the large over development seasons now ranges
from 0.9835 to 1.0134 (`lgbm`: 0.9766 to 1.0198), and 2024-25 moves from 0.9714 to 0.9934.
`spline_level` meets F-f on validation (ECE 0.0097) but not on test (0.0120). The G-g condition
(F-f on validation and a lower CV log loss than `lgbm`) does not hold, so team and player
numbers stay on `lgbm`. **Hypothesis:** one offset per season corrects the league-wide level,
but the failing reliability bins (P ≈ 0.40 and 0.69) come from the shape of the curve within
distance bands (long 2s, deep 3s), which a single additive shift cannot change.

## Runtime
`eurohoops backtest --model m2 --score-test` fits 582 models; the 330 leave-two-out LightGBM
fits for the nested isotonic calibration are about 69% of the time. Budget 60 minutes (the
user's decision, weeks 7–10b; it was 40). Number-preserving speed-ups (fits in two worker
processes, reused fits, zone codes once) bring it under budget with every weeks 7–10 number
byte-identical; details, per-phase times and the options that would change numbers are in
`docs/models/m2_runtime.md`.
...
- **Free throws:** season-level variation defeats the ±0.1 reconciliation target, and the
  re-specified share check fails too: the model has no team foul-drawing term.
- **Season-level variant:** fixes calibration in the large, not the F-f reliability bins
  (post-hoc on validation and test; the clean test is 2026-27).
- **Runtime:** under the relaxed 60-minute budget (see "Runtime").
...
| Number | Value | Source |
|---|---|---|
| lgbm_level cv log_loss | 0.629697 | `backtest_m2.json:level_variants.variants.lgbm_level.cv.log_loss` |
| lgbm_level validation log_loss | 0.633309 | `backtest_m2.json:level_variants.variants.lgbm_level.validation.log_loss` |
| lgbm_level validation ece | 0.0102 | `backtest_m2.json:level_variants.variants.lgbm_level.validation.ece` |
| lgbm_level test ece | 0.0095 | `backtest_m2.json:level_variants.variants.lgbm_level.test.ece` |
| spline_level cv log_loss | 0.631519 | `backtest_m2.json:level_variants.variants.spline_level.cv.log_loss` |
| spline_level validation log_loss | 0.635106 | `backtest_m2.json:level_variants.variants.spline_level.validation.log_loss` |
| spline_level validation ece | 0.0097 | `backtest_m2.json:level_variants.variants.spline_level.validation.ece` |
| spline_level test ece | 0.0120 | `backtest_m2.json:level_variants.variants.spline_level.test.ece` |
| lgbm_level - lgbm validation log loss | 0.000042 | `backtest_m2.json:level_variants.variants.lgbm_level.minus_base.validation.log_loss.mean` |
| lgbm_level - lgbm validation CI low | -0.000049 | `backtest_m2.json:level_variants.variants.lgbm_level.minus_base.validation.log_loss.ci95.0` |
| lgbm_level - lgbm validation CI high | 0.000139 | `backtest_m2.json:level_variants.variants.lgbm_level.minus_base.validation.log_loss.ci95.1` |
| lgbm_level k (validation/test) | 2312 | `backtest_m2.json:level_variants.variants.lgbm_level.shrinkage.later.k` |
| lgbm_level calibration in the large 2013 | 0.9835 | `backtest_m2.json:level_variants.variants.lgbm_level.calibration_in_the_large.level.2013` |
| lgbm_level calibration in the large 2011 | 1.0134 | `backtest_m2.json:level_variants.variants.lgbm_level.calibration_in_the_large.level.2011` |
| lgbm calibration in the large 2024 | 0.9714 | `backtest_m2.json:level_variants.variants.lgbm_level.calibration_in_the_large.base.2024` |
| lgbm_level calibration in the large 2024 | 0.9934 | `backtest_m2.json:level_variants.variants.lgbm_level.calibration_in_the_large.level.2024` |
